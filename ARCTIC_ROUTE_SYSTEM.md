...
| A | 下载、留证、拆帧、标准化、QC、持久化和回放 | 0.4.2；工程门禁已通过 | 冻结可重复演示数据集 |
...
| D | 只读展示风险、路线和指标 | 尚未实现 | 完成挑战杯可视化闭环 |
| orchestrator | 只经公共 API 组织 A→B→C、报告和制品 | 环境已补齐；长集成待分段 | 形成稳定演示脚本和失败报告 |
...
| 项目 | 当前值 | 用途 |
|---|---|---|
| `corridor_id` | `tromso_to_isfjorden_outer` | 主线完成后的迁移验证 |
...
顺序固定为：**先摩尔曼斯克—迪克森，后迁移特罗姆瑟—伊斯峡湾外部入口。**短航线仍可用于
快速 smoke，但不能改变产品开发顺序。
...
仍需记录但本轮不额外处理的风险：编排器长运行性能与阶段遥测；CNN P2/P3；规则和船型未科学
校准；D 尚未实现；外部数据源和凭据会变化；bathymetry、法规区和 hard mask 缺少完整证据；
真实长窗主线仍未贯通。它们不应被删除，也不应自动升级为当前开发任务。
