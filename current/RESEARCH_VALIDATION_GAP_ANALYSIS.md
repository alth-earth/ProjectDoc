...
| A | 12 类型采集/规整化/QC/溯源、PreparedWindow、DatasetBundle.v2、精确解析器、CARRA 与冰数据接口 | RC1/RC2 summer bundles；Winter 12/12 冻结 bundle；CARRA/Copernicus/GEBCO 记录 | Winter bundle 对 144 h 场景最小 horizon 仅 132 h；无合法 RunContext | 发布新的不可变 144 h 最小 bundle 标识，再创建 RunContext/接入 |
...
## A 与 Winter 场景（2026-08-22 22:24 +08:00）

A 可采集并表达 12 个必需 Winter 类型，含显式 UTC 窗口、溯源与因果可见性。CARRA/Copernicus/GEBCO 源集已作为冻结的 1,212 条记录 bundle 发布。其请求窗口为 144 h，但最小 horizon 仅 132 h；既有 RunContext 与 Orchestrator 接入契约拒绝该不匹配。因此：
...
- winter 实验标识：`BLOCKED_BY_BUNDLE_MINIMUM_HORIZON`；
