...
| 候选航线展示 | Orchestrator 投影；C 拥有候选语义 | Orchestrator | D Viewer | `presentation.route-candidates.v1` | FROZEN_COMPATIBLE | 支持真实完整 4×3 `PUBLISHED` sidecar；缺失时保持 `NOT_PUBLISHED` + 空列表 |
...
| [逐格风险解释 sidecar](../../reports/research-validation/RISK_EXPLANATION_SIDECAR_DESIGN_REPORT.md)（[schema](../proposals/risk-explanation.v1.schema.json)） | DRAFT contract；B 同次公式 trace + immutable `risk-explanation.v1` artifact/manifest `ENGINEERING_CHAIN_PASS`（`demo_unvalidated`）；Orchestrator SHA/identity transport `PASS`；D optional consumer `BROWSER_E2E_PASS`；不修改 `bc.risk-frame.v2` | B 风险语义负责人 + Orchestrator + D |
